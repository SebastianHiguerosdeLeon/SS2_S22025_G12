...
-- 3. Crear una tabla copia con datos de enero–marzo, limpia y optimizada
...
-- 4. Ver las particiones creadas en la tabla copia
...
-- 5. Revisar nulos en columnas clave
SELECT
...
/* ============================================================
   1. TABLA BASE LIMPIA (Q1) → trips_q1_clean
   ============================================================ */
CREATE OR REPLACE TABLE `semi2-proyecto-g12.fase1.trips_q1_clean`
PARTITION BY DATE(pickup_datetime)
CLUSTER BY pickup_location_id, dropoff_location_id, payment_type AS
SELECT *
FROM `bigquery-public-data.new_york_taxi_trips.tlc_yellow_trips_2022`
...
FROM `semi2-proyecto-g12.fase1.trips_q1_clean`
...
/* ============================================================
   4. DEMANDA POR HORA (Q1) → demanda_horaria_q1
   ============================================================ */
CREATE OR REPLACE TABLE `semi2-proyecto-g12.fase1.demanda_horaria_q1`
PARTITION BY DATE(pickup_datetime)
...
SELECT
  ...
  COUNT(*) AS trips
FROM `semi2-proyecto-g12.fase1.trips_q1_clean`
GROUP BY trip_date, hour_of_day, pickup_location_id;

-- Chequeo esquema
SELECT column_name, data_type
FROM `semi2-proyecto-g12.fase1.INFORMATION_SCHEMA.COLUMNS`
WHERE table_name = 'demanda_horaria_q1';
